...
browse.prototype.scrollY = function(toY, duration) {
  ...
  var tick = function() {
    ...
    else if('up' === direction && currY < toY) {
      currY = toY
    }
    element.scrollTo(currX, currY)
    if('down' === direction && currY < toY || 'up' === direction && currY > toY) {
      setupAnimationFrame(tick)
    }
  }
  tick()
...
function setupAnimationFrame(func) {
  (window.requestAnimationFrame && window.requestAnimationFrame(func)) || setTimeout(func, __frame_interval__)
}
